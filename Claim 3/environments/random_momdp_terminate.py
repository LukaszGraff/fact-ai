...
class RandomMOMDP(gym.Env):
    ...
    def __init__(self, seed=42, goal_states=None):
        super().__init__()
        ...
        if goal_states is not None:
            ...
            self.goal_states = self.np_random.choice(np.arange(1, 50), size=3, replace=False)
    ...
    def _generate_rewards(self):
        ...
        for goal_idx, goal_state in enumerate(self.goal_states):
    ...
    def step(self, action: int):
        ...
        next_state = self.np_random.choice(self.num_states, p=next_state_dist)
        self.state = next_state

        # Reward depends on the STATE YOU ENTER
        reward_vector = self.R[next_state, action].copy()

        # Terminate if goal reached OR max steps
        reached_goal = next_state in self.goal_states
...
gym.register(
    id="RandomMOMDP-v1",
    entry_point="environments.random_momdp_terminate:RandomMOMDP",
)
