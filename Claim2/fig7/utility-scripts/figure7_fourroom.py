...
def _cell_indices_from_flat(cell_index: int, grid_points: int) -> tuple[int, int]:
    if cell_index < 0 or cell_index >= grid_points * grid_points:
        raise ValueError(
            f"cell_index must be in [0, {grid_points * grid_points}). Got {cell_index}."
        )
    i = cell_index // grid_points
    j = cell_index % grid_points
    return i, j


def _cell_dir(save_dir: str, d2: float, d3: float) -> str:
    return os.path.join(save_dir, f"mu2_{d2:+.3f}_mu3_{d3:+.3f}")


def _save_cell_result(
    path: str,
    *,
    d2: float,
    d3: float,
    ...
    np.savez(
        ...
        fixed_mu=np.asarray(fixed_mu, dtype=np.float32),
        nsw=np.asarray(nsw, dtype=np.float64),
        util=np.asarray(util, dtype=np.float64),
        jain=np.asarray(jain, dtype=np.float64),
        ...
        grid_points=np.asarray(grid_points, dtype=np.int32),
    )
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=-1,
        help="Run a single cell by flattened index in [0, grid_points^2).",
    )
    parser.add_argument("--d2", type=float, default=None, help="Run a single cell with explicit d2.")
    parser.add_argument("--d3", type=float, default=None, help="Run a single cell with explicit d3.")
    parser.add_argument(
        "--cell_output",
        type=str,
        default="cell_result.npz",
        help="Filename for per-cell outputs inside each cell directory.",
    )
    ...
    explicit_cell = args.d2 is not None or args.d3 is not None
    indexed_cell = args.cell_index >= 0
    if explicit_cell and indexed_cell:
        raise ValueError("Use either --cell_index or (--d2 and --d3), not both.")
    if explicit_cell and (args.d2 is None or args.d3 is None):
        raise ValueError("When using explicit cell mode, both --d2 and --d3 are required.")

    # Single-cell mode: run exactly one perturbation cell and exit.
    if indexed_cell or explicit_cell:
        if indexed_cell:
            i, j = _cell_indices_from_flat(args.cell_index, args.grid_points)
            d2 = float(deltas[i])
            d3 = float(deltas[j])
        else:
            d2 = float(args.d2)
            d3 = float(args.d3)
            i_matches = np.where(np.isclose(deltas, d2))[0]
            j_matches = np.where(np.isclose(deltas, d3))[0]
            if len(i_matches) == 0 or len(j_matches) == 0:
                raise ValueError(
                    "Explicit d2/d3 must match the grid defined by "
                    f"[grid_min={args.grid_min}, grid_max={args.grid_max}, grid_points={args.grid_points}]. "
                    f"Got d2={d2}, d3={d3}."
                )
            i = int(i_matches[0])
            j = int(j_matches[0])
        ...
        fixed_mu = np.array(
            [mu_star[0], mu_star[1] * (1.0 + d2), mu_star[2] * (1.0 + d3)],
            dtype=np.float32,
        )
        run_dir = _cell_dir(args.save_dir, d2, d3)
        ...
        cell_path = os.path.join(run_dir, args.cell_output)
        _save_cell_result(
            cell_path,
            d2=d2,
            d3=d3,
        ...
        print(
            f"Saved single-cell result to {cell_path} "
            f"(i={i}, j={j}, d2={d2:+.3f}, d3={d3:+.3f}, seed={args.seed})."
        )
        return
    ...
    for i, d2 in enumerate(deltas):
        for j, d3 in enumerate(deltas):
            ...
            run_dir = _cell_dir(args.save_dir, float(d2), float(d3))
            os.makedirs(run_dir, exist_ok=True)
            cfg = SimpleNamespace(**vars(config))
            cfg.fixed_mu = fixed_mu
            cfg.seed = args.seed
