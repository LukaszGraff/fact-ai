...
class MuNetwork(nnx.Module):
    def __init__(self,
                 config):
        if hasattr(config, "mu_init") and config.mu_init is not None:
            init_array = jnp.asarray(config.mu_init, dtype=jnp.float32)
            if init_array.shape == ():
                init_array = jnp.full((config.reward_dim,), init_array)
            elif init_array.shape != (config.reward_dim,):
                raise ValueError(
                    f"mu_init must have shape ({config.reward_dim},), got {init_array.shape}"
                )
            self.mu = nnx.Param(init_array)
        else:
