...
def main():
    ...
    parser.add_argument("--mu_init", type=str, default=None, help="Comma-separated mu initialization values")
    ...
    if config.mu_init is not None:
        parts = [p.strip() for p in config.mu_init.split(",") if p.strip() != ""]
        if len(parts) == 0:
            raise ValueError("--mu_init provided but no values were parsed")
        if len(parts) == 1:
            config.mu_init = [float(parts[0])] * config.reward_dim
        else:
            if len(parts) != config.reward_dim:
                raise ValueError(
                    f"--mu_init expects {config.reward_dim} values, got {len(parts)}"
                )
            config.mu_init = [float(p) for p in parts]
    ...
    elif config.learner == "FairDICE_Fixed":
        from FairDICE_Fixed import init_train_state, train_step, get_model, save_model, load_model
    else:
    ...
    for iter in tqdm(range(train_iterations), desc="Training ..."):
        ...
        mu_last = np.array(update_info["mu"][-1])
        last_mu = mu_last
        print(f"mu: {mu_last}", flush=True)
    ...
    if config.save_path:
        ...
        if last_mu is not None:
            mu_path = os.path.join(save_dir, "final_mu.txt")
            with open(mu_path, "w") as f:
                f.write(",".join([str(x) for x in last_mu.tolist()]) + "\n")
