...
def sample_simplex_dirichlet(n_obj: int, n: int, seed: int = 0):
    rng = np.random.default_rng(seed)
    return rng.dirichlet(alpha=np.ones(n_obj), size=n)
...
def experiment(
    ...
    data_path = '../data/'
    dataset_paths = [f"{data_path}/{env_name}/{env_name}_50000_{d}.pkl" for d in dataset]
    ...
    traj_lens = np.asarray(traj_lens)
    returns = np.asarray(returns)
    returns_mo = np.asarray(returns_mo)
    ...
    states = np.concatenate(list(states), axis=0)
    ...
    eval_pref_mode = variant.get('eval_pref_mode', 'grid')
    num_eval_prefs = variant.get('num_eval_prefs', None)
    pref_seed = variant.get('pref_seed', 0)

    if eval_pref_mode == 'dirichlet':
        if num_eval_prefs is None:
            raise ValueError("--num_eval_prefs must be set when --eval_pref_mode dirichlet")
        prefs = sample_simplex_dirichlet(pref_dim, num_eval_prefs, seed=pref_seed)
    else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--eval_pref_mode', type=str, default='grid', choices=['grid', 'dirichlet'])
    parser.add_argument('--num_eval_prefs', type=int, default=None)  # used if eval_pref_mode=dirichlet
    parser.add_argument('--pref_seed', type=int, default=0)
