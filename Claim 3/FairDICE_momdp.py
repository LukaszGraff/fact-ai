...
def train_step(config, train_state: TrainState, batch, key: jax.random.PRNGKey):
    ...
    def nu_loss_fn(nu_network, mu_network):
        ...
        mu = jnp.clip(mu_network(), 1e-4, 10.0)  # 🔥 SAFE bounds
        k = 1.0 / mu  # Safe division
        ...
        loss_3 = config.alpha * jnp.sum(
            jnp.log(jnp.clip(k, 1e-4, 1e4)) - mu * k
        )
    ...
    (nu_loss, (w, e, mu, grad_penalty)), (nu_grads, mu_grads) = nnx.value_and_grad(nu_loss_fn, argnums=(0, 1),
                                                                                   has_aux=True)(nu_network, mu_network)
    ...
    def policy_loss_fn(policy):
        ...
        if is_discrete:
            ...
            # 🔥 SHAPE FIX:
            actions_int = batch.actions.astype(jnp.int32).squeeze()  # Ensure (64,)
            batch_idx = jnp.arange(log_probs.shape[0])  # (64,)
            log_probs = log_probs[batch_idx, actions_int]  # (64,) ✓
            log_probs = log_probs.reshape(-1, 1)  # (64,1) ✓
