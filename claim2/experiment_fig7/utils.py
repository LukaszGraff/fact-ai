...
try:
    ...
    import gym
try:
    import d4rl  # Optional; only required for d4rl datasets.
except ModuleNotFoundError:
    d4rl = None
try:
    import environments  # Optional; used for non-fig7 datasets.
except ModuleNotFoundError:
    environments = None
import numpy as np
import pickle
...
class MOOfflineEnv(Wrapper):
    def __init__(self, env_name, dataset='d4rl', num_objective=2, reset_noise_scale=1e-3):
        ...
        if dataset == 'd4rl':
            if d4rl is None:
                raise ModuleNotFoundError("d4rl is required when dataset='d4rl'")
